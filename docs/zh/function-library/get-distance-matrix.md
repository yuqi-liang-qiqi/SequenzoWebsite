...
想系统了解各种算法怎么选、各参数什么意思，建议先看这份[《非相似度度量指南》](../zh/tutorials/dissimilarity-measures.md)。如果你先认真读完，再来阅读这个函数文档的话，就会省时又省力。
...
可用的算法有多种（[详见指南](../zh/tutorials/dissimilarity-measures.md)）。由于 `get_distance_matrix()` 需要同时支持所有这些算法，参数看起来会比较多。但在实际使用中，每一种相异度算法只需要少量的关键参数，刚刚我们括号里提到的指南对这些参数都有清晰的说明。
