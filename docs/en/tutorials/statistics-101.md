...
When people first learn statistics, they are often taught through models, such as linear regression, logistic regression, and multilevel models. These quickly become the center of attention. As a result, many beginners walk away with the impression that:
...
This guide clarifies what *is* a model, what is *not* a model, and where concepts like probability, random processes, sequence analysis, and Markov chains fit in. It starts with the basics and gradually moves to more complex ideas.
...
## Part 2: From Concepts to Models

Now that we understand what models are, we can separate a statistical concept from a statistical model. The distinction matters because a concept may describe dependence or randomness without specifying probabilities that can be estimated.
...
By itself, a random process is not yet a model. It tells us that we are dealing with sequences of random variables, but it does not specify:
...
Hidden Markov Models (HMMs) take one more step. They add a hidden-state layer that is useful when the observed states are noisy traces of an unobserved process.
...
## Part 3: Sequence Analysis as a Special Case
...
This distinction matters for beginners. Dissimilarity-based clustering in sequence analysis is best understood as a **descriptive and exploratory approach**, not a data-generating model.
...
Understanding sequence analysis as a **descriptive approach** helps avoid these misunderstandings.
...
* **Inputs for later modeling**: preparing data for outcome models, typology comparisons, or feature construction
...
Understanding this distinction helps you learn statistics more precisely.
...
For this reason, sequence analysis is best understood as a **descriptive and exploratory approach**, rather than a statistical model.
...
This checklist helps you classify statistical ideas before deciding how to use them.
...
- [Latent Class Analysis](./lca.md) applies these modeling ideas to sequences.
- [Markov Chains](./markov-chain-models-01.md) introduces the simplest sequence model.
- [Sequence Analysis vs. LCA vs. HMM](./sa-lca-and-hmm.md) compares the modeling options.
