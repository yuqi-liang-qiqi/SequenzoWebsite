...
Some individuals may follow a path such as school → job → marriage → children, while others may delay marriage, cohabit, or remain single.

When thinking this way, we are already making an implicit assumption: thousands of individuals in our data can be grouped into a few typical patterns or life-course types, often called typologies in the sequence analysis community.
...
At first glance, all three methods seem to aim for the same goal because they classify individuals with similar life courses. They rely on very different logics:
...
To calculate these differences, we use [dissimilarity measures](../function-library/get-distance-matrix.md), one of the most important components of sequence analysis. If we use optimal matching (OM), one of the most commonly used dissimilarity measures, it calculates how many small changes (insertions, deletions, substitutions) are needed to turn one sequence into another. It is a bit like measuring how many edits it takes to make two words look the same, which is why this kind of dissimilarity measure falls under the category of edit distance.
...
After calculating these distances for everyone in the data, we use **cluster analysis** to group people whose lives look alike. In practice, many sequence-analysis studies use hierarchical clustering. Ward methods are common in the literature, while average linkage is often a safer first choice for OM, LCS, and related sequence distance matrices because Ward assumes Euclidean distances.
...
Imagine you run a bakery that sells cupcakes with the following flavors:
...
You notice that customers tend to buy different combinations of flavors, but you do not know how many customer types there are or what each type prefers.
...
Now you want to infer the hidden taste types.
...
These numbers are just your *starting assumptions* because they are not based directly on the empirical data yet.
...
This is an **appearance-based classification** because it relies on measuring how close or distant two observed sequences are.

There is no assumption that these cities share an underlying data-generating mechanism.
The method only says: *these trajectories look alike*, not *these trajectories come from the same process*.
It provides an **empirical grouping**, based on observed similarity in shape or timing.
...
* **Latent Class Analysis (LCA) is parametric**:
  ...
  Mixture models are covered in a separate tutorial.
...
Unlike LCA, which assumes that time points are independent given the latent class, HMMs explicitly model temporal dependence between states. In other words, HMMs are **dynamic models** that pay close attention to the *timing* and *sequencing* of transitions, much like sequence analysis does.
...
These stages themselves cannot be directly observed. That is why the model is called *hidden*.
...
In life-course analysis, the hidden moods are the **unobservable life stages**. For example, a health model might distinguish good health, early decline, and chronic frailty.
...
This makes HMMs useful for studying **transitions and uncertainty**. For example, HMMs can address questions such as:
...
In short, dissimilarity-based clustering in sequence analysis compares observed paths, LCA groups people by overall patterns, and HMMs model the unobserved process that generates the sequence step by step.
...
Instead of assigning each person to one fixed class, HMMs assume that everyone moves through several latent phases during life. For example, a model might distinguish beginning to slow down from chronic limitation.
...
* HMMs see it as movement through hidden stages that unfold over time.
...
Overall, the number of groups is largely a **researcher’s choice**, guided by both data and theory. For these three methods, we recommend using both statistical indicators, which differ across methods, and substantive interpretation grounded in theory.

**1. Dissimilarity-based clustering in sequence analysis: Deciding by clustering and interpretation**

After computing pairwise distances between sequences, SA usually applies **hierarchical clustering** to group similar trajectories.
...
> Note: 
> We do not recommend keeping too many clusters (e.g., more than 10), because they can become difficult to interpret.
...
Typically, the model with the **lowest BIC** is preferred, as it captures the structure of the data without overfitting. Substantive interpretation also matters because you need to assess whether the resulting classes are meaningful, theoretically plausible, and substantively distinct.
...
| **Main tool**       | Distance matrix + cluster analysis                                        | Likelihood model + estimated class membership probabilities              | Markov chain with emission and transition matrices estimated via EM algorithm             |
...
* The **hidden process** is 
  ...
  * In other words, in an HMM, the hidden process is the underlying temporal process. For example, people move through unobserved life stages such as career building, family formation, or retirement preparation.
...
## 4. What each method is good at, and where it is limited
...
They found that both methods are fairly stable when sequences are clearly distinct, but LCA performs better when within-group randomness is large or when measurement error is present.

In other words, LCA was more stable with noisy data because it models probabilities and can tolerate random deviations around a latent pattern. SA, relying purely on exact sequence dissimilarities, became less stable as noise grew.
...
- [Latent Class Analysis](./lca.md) and [Hidden Markov Models](./markov-chain-models-02.md) cover each model in depth.
- [Markov Chain Models](/en/markov-chain-models/introduction) documents the HMM-family workflows in Sequenzo.
- [Cluster Analysis Methods](./cluster-analysis-methods.md) covers the dissimilarity-based side.
...
---
*We thank [Heyi Zhang](https://profiles.ucl.ac.uk/100967-heyi-zhang) for suggesting that we explain the differences between related methods in life course studies.*
