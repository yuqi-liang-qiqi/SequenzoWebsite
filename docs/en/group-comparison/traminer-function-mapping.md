...
| --- | --- | --- |
| [`conceptual-guide`](./conceptual-guide) | `TraMineRextras::seqCompare()` methodological framework | Conceptual explanation of one-group vs two-group comparison and interpretation of LRT/BIC evidence. |
| [`get_group_differences`](./get-group-differences) | `TraMineRextras::seqCompare()` | Main overall two-group comparison interface; returns LRT/BIC outputs depending on `stat`. |
| [`get_lrt_test`](./get-lrt-test) | `TraMineRextras::seqLRT()` | LRT-focused wrapper built on the same overall comparison engine. |
| [`get_bic_test`](./get-bic-test) | `TraMineRextras::seqBIC()` | BIC-focused wrapper built on the same overall comparison engine. |
